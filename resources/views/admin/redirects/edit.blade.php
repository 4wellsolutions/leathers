...
@section('content')
    <form action="{{ route('admin.redirects.update', $redirect) }}" method="POST" class="max-w-2xl mx-auto pb-20">
        @csrf
        @method('PUT')

        <div class="mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-leather-900">Edit Redirect</h1>
                <p class="text-sm text-neutral-500">Update existing redirection rule.</p>
            </div>
            <a href="{{ route('admin.redirects.index') }}"
                class="text-sm font-medium text-neutral-600 hover:text-neutral-900">Cancel</a>
        </div>

        <div class="bg-white rounded-xl shadow-sm border border-neutral-200 overflow-hidden p-6 space-y-6">
            <div>
                <label for="from_url" class="block text-sm font-medium text-neutral-700 mb-2">From URL (Relative)</label>
                <div class="flex rounded-lg shadow-sm">
                    ...
                    <input type="text" name="from_url" id="from_url" value="{{ old('from_url', $redirect->from_url) }}"
            ...
            <div>
                <label for="to_url" class="block text-sm font-medium text-neutral-700 mb-2">To URL (Relative or
                    Absolute)</label>
                <input type="text" name="to_url" id="to_url" value="{{ old('to_url', $redirect->to_url) }}"
            ...
            <div>
                ...
                <select name="status_code" id="status_code"
                    class="block w-full rounded-lg border-neutral-300 shadow-sm focus:border-gold-500 focus:ring-gold-500 sm:text-sm py-3 px-4">
                    <option value="301" {{ old('status_code', $redirect->status_code) == '301' ? 'selected' : '' }}>301 -
                        Permanent Redirect</option>
                    <option value="302" {{ old('status_code', $redirect->status_code) == '302' ? 'selected' : '' }}>302 -
                        Temporary Redirect</option>
                </select>
            </div>
            ...
            <div class="flex items-center justify-between pt-4 border-t border-neutral-100">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $redirect->is_active) ? 'checked' : '' }} class="sr-only peer">
                    <div
        ...
        <div class="mt-8 flex items-center justify-end">
            <button type="submit"
                class="px-6 py-3 text-sm font-medium text-white bg-leather-900 border border-transparent rounded-lg hover:bg-leather-800 shadow-sm">
                Update Redirect
            </button>
        </div>
    </form>
