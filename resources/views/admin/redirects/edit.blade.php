    ...
    <div class="bg-white rounded-xl shadow-sm border border-neutral-200 overflow-hidden p-6 space-y-6">
        <div>
            ...
            <div class="flex rounded-lg shadow-sm">
                ...
                <input type="text" name="old_url" id="old_url" value="{{ old('old_url', $redirect->old_url) }}" class="flex-1 min-w-0 block w-full px-4 py-3 rounded-none rounded-r-lg focus:ring-gold-500 focus:border-gold-500 sm:text-sm border-neutral-300">
            </div>
        </div>

        <div>
            <label for="new_url" class="block text-sm font-medium text-neutral-700 mb-2">New URL (Relative or Absolute)</label>
            <input type="text" name="new_url" id="new_url" value="{{ old('new_url', $redirect->new_url) }}" class="block w-full rounded-lg border-neutral-300 shadow-sm focus:border-gold-500 focus:ring-gold-500 sm:text-sm py-3 px-4">
        </div>
        ...
        <div>
            ...
            <select name="status_code" id="status_code" class="block w-full rounded-lg border-neutral-300 shadow-sm focus:border-gold-500 focus:ring-gold-500 sm:text-sm py-3 px-4">
                <option value="301" {{ old('status_code', $redirect->status_code) == '301' ? 'selected' : '' }}>301 - Permanent Redirect</option>
                <option value="302" {{ old('status_code', $redirect->status_code) == '302' ? 'selected' : '' }}>302 - Temporary Redirect</option>
            </select>
        </div>
