...
@section('content')
    <form action="{{ route('admin.email-templates.update', $emailTemplate->id) }}" method="POST" class="pb-20">
        @csrf
        @method('PUT')
        ...
        <div class="sticky top-0 z-10 bg-neutral-100/95 backdrop-blur-sm border-b border-neutral-200 -mx-4 sm:-mx-6 md:-mx-8 px-4 sm:px-6 md:px-8 py-4 mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-leather-900">Edit Email Template</h1>
                <p class="text-sm text-neutral-500">Update template information.</p>
            </div>
                ...
                <div class="p-6 md:p-8 space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $emailTemplate->name) }}" class="block w-full rounded-lg border-neutral-300 shadow-sm focus:border-gold-500 focus:ring-gold-500 sm:text-sm py-3 px-4">
                            <p class="mt-1 text-xs text-neutral-500">Unique identifier for this template</p>
                        </div>

                        <div>
                            <label for="subject" class="block text-sm font-medium text-neutral-700 mb-2">Email Subject</label>
                            <input type="text" name="subject" id="subject" value="{{ old('subject', $emailTemplate->subject) }}" class="block w-full rounded-lg border-neutral-300 shadow-sm focus:border-gold-500 focus:ring-gold-500 sm:text-sm py-3 px-4">
                        </div>

                        <div>
                            <label for="body" class="block text-sm font-medium text-neutral-700 mb-2">Email Body</label>
                            <textarea id="body" name="body" rows="12" class="block w-full rounded-lg border-neutral-300 shadow-sm focus:border-gold-500 focus:ring-gold-500 sm:text-sm py-3 px-4 font-mono text-sm">{{ old('body', $emailTemplate->body) }}</textarea>
                            <p class="mt-1 text-xs text-neutral-500">Use double curly braces for variables: {{variable_name}}</p>
                        </div>

                        <div>
                            <label for="variables" class="block text-sm font-medium text-neutral-700 mb-2">Available Variables</label>
                            <textarea id="variables" name="variables" rows="3" class="block w-full rounded-lg border-neutral-300 shadow-sm focus:border-gold-500 focus:ring-gold-500 sm:text-sm py-3 px-4">{{ old('variables', $emailTemplate->variables) }}</textarea>
                            <p class="mt-1 text-xs text-neutral-500">List available variables for reference</p>
                        </div>
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-neutral-100">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $emailTemplate->is_active) ? 'checked' : '' }} class="sr-only peer">
