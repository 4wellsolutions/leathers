...
@section('content')
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
            <h3 class="text-lg leading-6 font-medium text-leather-900">All Sales</h3>
                    ...
                    @foreach($sales as $sale)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-leather-900">{{ $sale->name }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-leather-900">
                                    ...
                                    @else
                                        Rs. {{ number_format($sale->discount_value) }} Off
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-neutral-500">
                                    {{ $sale->starts_at ? $sale->starts_at->format('M d, Y') : 'N/A' }} -
                                    {{ $sale->ends_at ? $sale->ends_at->format('M d, Y') : 'N/A' }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $sale->is_active && $sale->isValid() ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $sale->is_active && $sale->isValid() ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
